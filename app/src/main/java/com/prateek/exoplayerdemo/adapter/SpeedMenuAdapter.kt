...
class SpeedMenuAdapter(private val items: List<SpeedData>) :
    BaseAdapter() {
    override fun getCount(): Int {
        return items.size
    }

    override fun getItem(i: Int): SpeedData {
        return items[i]
    }
    ...
    override fun getView(position: Int, convertView: View?, parent: ViewGroup): View {
        ...
        if (convertView == null) {
            convertView =
                LayoutInflater.from(parent.context).inflate(R.layout.speed_popup_list_item, null)
            holder = ViewHolder(convertView)
            convertView.tag = holder
        } else {
            holder = convertView.tag as ViewHolder
        }
        holder.tvTitle.text = "${getItem(position).speed}X"
